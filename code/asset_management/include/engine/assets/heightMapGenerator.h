		...
		void Generate(std::filesystem::path relativeAssetPath, std::string fileName, int width, int height, int seed) {
			...
			for (unsigned int i = 0; i < height; ++i) {     // y
				for (unsigned int j = 0; j < width; ++j) {  // x
					...
					double n = GenerateNoise(10 * x, 10 * y, 0.8);

					// Wood like structure
					//double n = 20 * GenerateNoise(x, y, 0.8);
					//n = n - floor(n);

					// Map the values to the [0, 255] interval, for simplicity we use 
					// tones of grey
					data[index++] = floor(255 * n);
					data[index++] = floor(255 * n);
					data[index++] = floor(255 * n);
			...
			SaveHeightMap(relativeAssetPath, fileName, width, height, 3, data);
		}
	...
	private:
		...
		*/
		void SaveHeightMap(std::filesystem::path relativeAssetPath, std::string fileName, const int width, const int height, const int channels,  std::vector<unsigned char> data) {	
			...
			stbi_write_png((p.string() + separator() + fileName).c_str(), width, height, channels, data.data(), width * channels);
		}
	};
}