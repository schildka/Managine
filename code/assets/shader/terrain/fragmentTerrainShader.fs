...
uniform samplerCube pointDepthMap[MAX_NUM_POINTLIGHTS];
...
{    
	// get diffuse color
    vec3 color1 = texture(material.texture1, fs_in.TexCoords).rgb;
	vec3 color2 = texture(material.texture2, fs_in.TexCoords).rgb;
	vec3 color3 = texture(material.texture3, fs_in.TexCoords).rgb;

	vec3 tmpColor = mix(color2, color1, fs_in.Color.xyz);
	vec3 finalColor = mix(color3, tmpColor, fs_in.Color.w);
	
	// properties
    vec3 norm = normalize(fs_in.Normal);
    vec3 viewDir = normalize(viewPos - fs_in.FragPos);
    
    vec3 result = numOfLights.z != 0 ? CalcDirLight(dirLight, norm, viewDir, finalColor) : vec3(0.0, 0.0, 0.0);

    for(int i = 0; i < numOfLights.x; i++){
		result += CalcPointLight(i, pointLights[i], norm, fs_in.FragPos, viewDir, finalColor); 
	}

	for(int i = 0; i < numOfLights.y; i++){
		result += CalcSpotLight(i, spotLights[i], norm, fs_in.FragPos, viewDir, finalColor); 
	}
...
vec3 CalcDirLight(DirectionalLight light, vec3 normal, vec3 viewDir, vec3 finalColor)
{
    ...
    vec3 result = (ambient + (1.0 - shadow) * diffuse) * finalColor;// + CascadeIndicator;    
...
vec3 CalcPointLight(int i, PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir, vec3 finalColor)
{
    ...
    return (ambient + (1.0 - shadow) * diffuse) * finalColor;
}
