...
{    
    ...
    vec3 result = numOfLights.z != 0 ? CalcDirLight(dirLight, norm, viewDir, color) : vec3(0.0, 0.0, 0.0);

    for(int i = 0; i < numOfLights.x; i++){
		result += CalcPointLight(i, pointLights[i], norm, fs_in.FragPos, viewDir, color); 
	}

	for(int i = 0; i < numOfLights.y; i++){
		result += CalcSpotLight(i, spotLights[i], norm, fs_in.FragPos, viewDir, color); 
	}
